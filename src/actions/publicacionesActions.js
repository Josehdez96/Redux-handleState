...
export const traerPorUsuario = (key) => async (dispatch, getState) => {
  ...
  const { usuarios } = getState().usuariosReducer; //Recibo o puedo tener acceso al estado actual con getState
...
export const traerComentarios = (pub_key, com_key) => async (
  ...
  try {
    const respuesta = await axios.get(
      `https://jsonplaceholder.typicode.com/posts?postId=${seleccionada.id}`
    );
